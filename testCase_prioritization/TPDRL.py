...
def experiment(
    ...
    for i in range(start_cycle, end_cycle - 1):
        ...
        print(
            "Training agent with replaying of cycle "
            + str(i)
            + " with steps "
            + str(steps)
        )
        ...
        if first_round:
            ...
            agent_params = tp_agent.get_parameters()
            l = tp_agent.get_parameter_list()
            tp_agent.learn(
                total_timesteps=steps, reset_num_timesteps=True, callback=callback_class
            )
            tp_agent.learn()._train_step.gamma = 0.98
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", "--mode", help="[pairwise,pointwise,listwise] ", required=True
    )
    ...
    parser.add_argument(
        "-w", "--win_size", help="Windows size of the history", required=False
    )
    parser.add_argument("-t", "--train_data", help="Train set folder", required=True)
    parser.add_argument(
        "-f", "--first_cycle", help="first cycle used for training", required=False
    )
    parser.add_argument(
        "-c", "--cycle_count", help="Number of cycle used for training", required=False
    )
    parser.add_argument(
        "-l",
        "--list_size",
    ...
    parser.add_argument(
        "-o", "--output_path", help="Output path of the agent model", required=False
    )
    ...
    assert (
        supported_formalization.count(args.mode.upper()) == 1
    ), "The formalization mode is not set correctly"
    assert (
        supported_algo.count(args.algo.upper()) == 1
    ), "The formalization mode is not set correctly"
